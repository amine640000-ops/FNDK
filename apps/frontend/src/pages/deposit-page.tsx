...
export function DepositPage() {
  ...
  const submitDeposit = async () => {
    ...
    if (!parsedAmount || parsedAmount < 1) {
      toast.error(tt("Enter a deposit amount of at least 1."));
      return;
    }

    if (!selectedAssetSetting) {
      toast.error(tt("No active deposit asset is available."));
      return;
    }
    ...
    try {
      ...
      toast.error(getApiErrorMessage(error, tt("Could not submit deposit.")));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const copyWalletAddress = async () => {
    ...
    toast.success(tt("Wallet address copied."));
  };
          ...
          <div className="absolute inset-x-4 bottom-6 rounded-[30px] border border-cyan-300/15 bg-[linear-gradient(180deg,rgba(14,19,142,0.98)_0%,rgba(9,11,110,0.99)_45%,rgba(6,8,82,1)_100%)] p-5 shadow-[0_28px_60px_rgba(0,0,0,0.4)]">
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <div className="mt-1 text-sm text-slate-300">{tt("Select the destination funding network.")}</div>
              </div>
              <button
            ...
            <div className="mt-5 space-y-3">
              {selectableAssetSettings.length ? selectableAssetSettings.map((assetSetting) => (
                ...
                <div className="rounded-[22px] border border-amber-300/20 bg-amber-300/10 px-4 py-4 text-sm text-amber-100">
                  {tt("No deposit assets are enabled by admin.")}
                </div>
              )}
            </div>
          </div>
      ...
      <div className="px-4 pt-5">
        <section className="neon-panel p-5">
          <div className="flex items-start gap-3">
            ...
            <div>
              <div className="text-[1.15rem] font-bold text-white">{tt("Create a deposit request")}</div>
              <div className="mt-1 text-[13px] leading-5 text-slate-300">
                {tt("Choose the asset and amount. The destination wallet address will be returned after submission.")}
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="neon-soft-panel mt-5 grid gap-4 p-5">
          ...
          <div className="rounded-[20px] border border-cyan-300/20 bg-cyan-300/10 px-4 py-4 text-[13px] leading-5 text-cyan-100">
            {tt("Request total")}: {formatCurrency(parsedAmount || 0)}. {tt("The destination wallet address will be returned after submission.")}
          </div>

          {selectedWalletAddress ? (
            <div className="rounded-[20px] border border-white/10 bg-white/5 p-4">
              <div className="text-sm text-slate-400">{tt("Funding address")}</div>
          ...
          <button
            ...
            type="button"
          >
            {submitting ? tt("Submitting...") : tt("Submit deposit")}
          </button>
        </section>
        ...
        {depositResponse ? (
          <section className="neon-panel mt-5 p-5">
            <div className="flex items-center gap-2 text-cyan-200">
              ...
              <span className="text-sm uppercase tracking-[0.22em]">{tt("Deposit request created")}</span>
            </div>

            <div className="mt-4 rounded-[22px] border border-white/10 bg-white/5 p-4">
              <div className="text-sm text-slate-400">{tt("Wallet address")}</div>
              <div className="mt-2 break-all text-[15px] font-semibold text-white">{selectedWalletAddress}</div>
              <button
                className="mt-4 inline-flex items-center gap-2 rounded-full border border-cyan-300/25 bg-cyan-300/10 px-4 py-2 text-sm font-semibold text-cyan-100"
                onClick={() => void copyWalletAddress()}
            ...
            <div className="mt-4 rounded-[22px] border border-white/10 bg-white/5 p-4 text-[13px] leading-6 text-slate-300">
              <div>{tt("Transaction ID")}: <span className="font-medium text-white">{depositResponse.id}</span></div>
              <div className="mt-1">{tt("Status")}: <span className="font-medium uppercase text-cyan-100">{depositResponse.status}</span></div>
